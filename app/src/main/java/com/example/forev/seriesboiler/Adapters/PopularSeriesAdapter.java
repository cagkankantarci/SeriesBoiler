...
public class PopularSeriesAdapter extends RecyclerView.Adapter<PopularSeriesAdapter.MyViewHolder> {

    Context context;
    Fragment fragment;
    List<SeriesModel> seriesList;

    public PopularSeriesAdapter(Context context, List<SeriesModel> seriesList) {
    ...
    @NonNull
    @Override
    public PopularSeriesAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view = LayoutInflater.from(context).inflate(R.layout.item_popular_series,viewGroup,false);
        return new MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final PopularSeriesAdapter.MyViewHolder myViewHolder, final int i) {
        myViewHolder.popularTitle.setText(seriesList.get(i).getTitle().toString());
